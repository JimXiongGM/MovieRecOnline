...
@author : Gong RY
'''
import MySQLdb,distance

def run_cal():
    db = MySQLdb.connect(host="localhost", port=3308, user="root", passwd="你的密码", db="MovieSizer", charset="utf8")
    cursor = db.cursor()
    ...
    for i in range(0, int(len(res)/100)):
        for j in range(i+1, len(res)):
            ...
            length = moviename_length + nation_length + directors_length + leadactors_length + editors_length
            similar = 1/length
            sql = "INSERT INTO movies_moviesimilar VALUES (%d, %d, %d, %f)" % (id, i_id, j_id, similar)
            cursor.execute(sql)
            db.commit()
            id = id +1
        print('current : ',i)
